	...
	*/
	if (message.content.startsWith("!start")) {
		...
		message.channel.send(gameHeadcountEmbed).then((headcountMSG) => {
			headcountMSG.react('⚪').then(() => headcountMSG.react('❌'));
			...
			//	var endTimeS = currTime.getSeconds;
			//}
			const headcountFilter = (reaction) => {
				return ['⚪'].includes(reaction.emoji.name) /*&& user.id === message.author.id*/;
			};
			headcountMSG.awaitReactions(headcountFilter, {
				max: 20,
				time: 180000,
				errors: ['time'],
			})
				.then(collected => {
					//const headcountReaction = collected.first();
					//playerNum++;
					//console.log("Reaction logged");
					message.channel.send("Too many people, please divide into to groups and start two separate games");
				})
				.catch(collected => {
...
client.login("NzMxOTMzNTY2OTI5MzM4NDU4.Xw8-cg.SdFg9puMWaFTaVzLpt_9juTKGzU");